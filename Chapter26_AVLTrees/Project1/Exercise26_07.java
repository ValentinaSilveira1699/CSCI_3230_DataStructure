...
public class Exercise26_07 {
    public static void main(String[] args) {
        ...
        // Check if the tree is an AVL tree and print the result
        boolean isAVL = tree.isAVLTree();
        System.out.println("Is this tree an AVL tree? " + isAVL);
    }
}
...
class MyBST<T> extends BST<T> {
    ...
    // Returns true if the tree is an AVL tree at the specified root within the specified range
    private boolean isAVLTree(TreeNode<T> root, int minValue, int maxValue) {
        if (root == null)
            return true;

        int heightDiff = Math.abs(height(root.left) - height(root.right));

        // Check if the current node violates the AVL property
        if (heightDiff > 1 || (Integer.parseInt(root.element.toString()) < minValue) ||
                (Integer.parseInt(root.element.toString()) > maxValue))
            return false;

        // Check recursively for the left and right subtrees
        return isAVLTree(root.left, minValue, Integer.parseInt(root.element.toString()) - 1)
                && isAVLTree(root.right, Integer.parseInt(root.element.toString()) + 1, maxValue);
    }
}
